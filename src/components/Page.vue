    ...
    >
      ...
      >
        ...
        <slot v-if="pageInfo.key === 'ae_3'">
          <p>921地震專題的「<router-link @click.native="handleGA('Room', 'Click', '921')" style="color: #38bee3" :to="`/0/28`">寫一封信給921</router-link>」推出後，有317位讀者在網頁上分享921經歷，甚至有讀者在漸漸被遺忘的信紙、稿紙上寫下一行行對921的回憶、郵寄到聯合報，在數位時代，用紙本傳遞著手機沒辦法給予的溫度。</p>
        </slot>
        <!-- 選舉題 -->
        ...
        <slot v-if="pageInfo.key === 'h_6'">
          <p>你以為<router-link @click.native="handleGA('Room', 'Click', 'Marriage')" style="color: #ff7fa1" :to="`/3/3`">兩岸婚姻</router-link>對數減少後，大家就對兩岸婚姻沒興趣了嗎？錯！兩岸婚姻專題在台灣和大陸「今日頭條」都引起熱烈討論。即使現在願意嫁來台灣的陸女少了，大家還是很愛看兩岸夫妻的相處故事（是在看八點檔嗎 XD）。</p>
        </slot>
      </ObservatoryEggPage>
    </div>
    ...
    >
      ...
      >
        ...
        <slot v-if="pageInfo.key === 'l_3'">
          <p>還記得到南投縣採訪921地震重建那天，龍眼林協會總幹事<router-link @click.native="handleGA('Theater', 'Click', 'Cindy_Liao')" style="color: #38bee3" :to="`/0/9`">廖振益</router-link>踩著夾腳拖、熱情招呼，協會在震後為孤老們送餐20年，廖振益的退休老本全砸進去了，但他笑笑的跟我們說：「沒關係啦，我這樣很像『廢物再利用捏』」。</p>
          <p><br></p>
        ...
        <slot v-if="pageInfo.key === 't_3'">
          <p>製作<router-link  @click.native="handleGA('Theater', 'Click', 'Pei_Tiger')" style="color: #bbd500" :to="`/2/6`">石虎</router-link>專題時，取得一份苗栗縣石虎路殺地點資料。為了呈現石虎路殺與道路開發的相關性，製作團隊周末加班趕工，花了一番功夫將座標資料轉成可用的格式，用QGIS標在地圖上，再套疊路網圖，找出石虎路殺熱區。當時國內還沒有其他媒體將這份資料完整呈現。</p>
          <p><p></p>
        ...
        <slot v-if="pageInfo.key === 'z_3'">
          <p>吼(ノ‵□′)ノ～┴─┴ </p>
          <p><router-link @click.native="handleGA('Theater', 'Click', 'Freddy_Privacy')" style="color: #ff7fa1" :to="`/3/16`">數位隱私</router-link>都還沒寫完，</p>
          <p>下下下題長官明天要看，</p>
          <p>還好沒日沒夜沒有<router-link @click.native="handleGA('Theater', 'Click', 'Freddy_Tiger')" style="color: #bbd500" :to="`/2/6`">石虎</router-link>畫成豹🐆🐆🐆</p>
          <p><br></p>
          <p>年度專題照片難，首頁封面自己來，</p>
          <p>左手攝影📸，右手寫code⌨️，</p>
          <p>專題滿滿<router-link @click.native="handleGA('Theater', 'Click', 'Freddy_921')" style="color: #38bee3" :to="`/0/2`">天亮之後</router-link>。</p>
          <p>灑花(*￣▽￣)/‧★*"`'*-.,_,.-*'`"*-.,_☆,.-*'`"*-.,_</p>
        </slot>      
        <!-- 藹雯 -->
        ...
        <slot v-if="pageInfo.key === 'm_5'">
          <p>製作<router-link @click.native="handleGA('Theater', 'Click', 'Mi_Tiger')" style="color: #bbd500" :to="`/2/6`">石虎</router-link>專題時，內容多次提到路殺熱區在省道、縣道，但光看文字很難聯想這些路殺有多嚴重。收到苗栗石虎陸殺紀錄後，結合點座標位置與路線地圖，才看到路殺的全貌，並把狀況呈顯給讀者瞭解。</p>
        </slot>
        <!-- 瑋琳 -->
        <slot v-if="pageInfo.key === 'q_6'">
          <p>今年很特別的是從視覺設計跨界到PM，以視覺的角度發動<router-link @click.native="handleGA('Theater', 'Click', 'Wei_DUI')" style="color: #ff7fa1" :to="`/3/7`">酒駕插畫</router-link>專題。</p>
          <p><br></p>
        ...
        <slot v-if="pageInfo.key === 'w_6'">
          <p>去南韓採訪<router-link @click.native="handleGA('Theater', 'Click', 'Afore_Energy')" style="color: #ff7fa1" :to="`/3/19`">能源轉型</router-link>專題時，一度以為自己回不來了。</p>
          <p><br></p>
